...
class DatabaseConnection {
     ...
     * @return int
     */
    public function getStoredBarcodeAmountFromBarcode(string $barcode): int {
        $res = $this->db->query("SELECT * FROM Barcodes WHERE barcode='$barcode'");
        if ($row = $res->fetchArray()) {
            return $row['amount'];
        } else {
     ...
     * @return void
     */
    public function insertUnrecognizedBarcode(?string $barcode, ?string $productname = null, ?string $altNamesGiven = null, int $amount = 1, string $bestBeforeInDays = null, string $price = null) {
